...
const UserNavInfo = ({ data, setData }: {
  ...
  const [forReload, setReload] = useState(1);
  useEffect(() => {
    setReload(forReload => 1 - forReload);
  ...
  return (<Link to="/user" className="btn bg-light justify-content-center border align-items-center border-dark rounded m-1 p-1 d-flex fw-bold">
    <div className="d-none d-sm-block">
      <img alt="" src={`http://localhost:8000/common/profile?${forReload}`} height={100} width={100} className="p-1 m-1" />
    </div>
    {data.username}
  </Link>);
};
